...
def low():
    bot.sendMessage(groupchatid, f"Temperature below 2 at {read_temp()}")
...
try:
    ...
    while 1:
        ...
        if current > 8 and status == False:
            alarm_high = schedule.every(15).minutes.do(high).tag('warnings')
            status = True
            daily = True
        if current < 2 and status == False:
            alarm_low = schedule.every(15).minutes.do(low).tag('warnings')
            status = True
            daily = True
